 ...
 */
function getAvailableYears($mysqli) {
    $query = "SELECT DISTINCT year FROM financial_records ORDER BY year DESC";
    $result = $mysqli->query($query);
    
    if (!$result) {
        error_log('Error fetching years: ' . $mysqli->error);
        return [date('Y')];
    }
...
$available_years = getAvailableYears($mysqli);
$financial_data = getFinancialDataFromDB($mysqli, $selected_year);
$stats = calculateStatsFromDB($mysqli, $selected_year);
  ...
  <div class="cards-area">
    ...
    <div class="stat-row">
      <div class="stat-card sc-green">
        <div class="stat-card-left">
          ...
          <span class="stat-trend up">↑ <?php echo $stats['revenue_growth']; ?>%</span>
        </div>
      ...
      <div class="stat-card sc-red">
        <div class="stat-card-left">
          ...
          <div class="stat-value"><?php echo formatCurrency($stats['total_expenses']); ?></div>
          <span class="stat-trend down">↑ <?php echo $stats['expense_growth']; ?>%</span>
        </div>
      ...
      <div class="stat-card sc-blue">
        <div class="stat-card-left">
          ...
          <div class="stat-value"><?php echo formatCurrency($stats['net_profit']); ?></div>
          <span class="stat-trend up"><?php echo $stats['profit_growth']; ?>%</span>
        </div>
      ...
      <div class="stat-card sc-gold">
        <div class="stat-card-left">
          ...
          <div class="stat-value"><?php echo $stats['roi']; ?>%</div>
    ...
    <div class="two-col">
      ...
      <div class="card" style="flex:1;">
        ...
        <div class="legend-list" style="margin-left:12px;">
          <?php 
            ...
            <?php if (!empty($financial_data['pnl_summary'])): ?>
              <?php foreach ($financial_data['pnl_summary'] as $row): ?>
                <tr>
                  ...
                  <td style="color:<?php echo str_contains($row[5], '▲') ? 'var(--success)' : (str_contains($row[5], '▼') ? 'var(--danger)' : 'var(--text-soft)'); ?>; font-weight:600;">
                    <?php echo $row[5]; ?>
                  </td>
                </tr>
              <?php endforeach; ?>
...
<script>
  ...
  const chartData = <?php echo json_encode($financial_data); ?>;
  ...
  function initializeCharts() {
    ...
    // Ensure we have data
    if (rev.length === 0) {
      console.log('No financial data available for selected year');
      return;
    }
  ...
  function initPLChart(revenue, expenses, profit) {
    ...
    if (plChartInstance) {
      plChartInstance.data.datasets[0].data = revenue;
      plChartInstance.data.datasets[1].data = expenses;
  ...
  function initRevenueMixChart() {
    ...
    if (revMixChartInstance) {
      revMixChartInstance.data.labels = revenueMixLabels;
      revMixChartInstance.data.datasets[0].data = revenueMixData;
